...
export const AiVisionPanel: React.FC<AiVisionPanelProps> = ({ latest }) => {
  ...
  const prediction = latest?.vision_prediction || 'HEALTHY';
      ...
      <div>
        ...
        <div className="bg-[#F2F6F4] border border-[#E2EAE6] rounded-xl p-3.5 mb-4 flex justify-between items-center">
          <div>
            <span className="text-[10px] uppercase font-semibold text-[#5C736B] block">
              Dominant Visual Indication
            </span>
            <span className="text-sm font-bold text-[#17332B] font-display capitalize">
              {prediction}
            </span>
          </div>
          <span className="text-[11px] font-medium text-[#2F6F5E] bg-white px-2.5 py-1 rounded-lg border border-[#E2EAE6]">
            TFLite Vision
          </span>
        </div>
        ...
        <div className="space-y-3.5 my-2">
          {bars.map((bar) => {
            ...
            return (
              <div key={bar.label}>
                <div className="flex justify-between text-xs font-semibold text-[#17332B] mb-1">
                  <span>{bar.label}</span>
